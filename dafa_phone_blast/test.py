...
base_url = "https://www.jihaoba.com/haoduan/139/tumushuke.htm"
...
s = requests.session()
s.keep_alive = False
local_res = requests.get(url=base_url,
                         headers=get_user_agent(), timeout=10).text
local_soup = BeautifulSoup(local_res, "lxml")
del (local_res)
for num in local_soup.select(".hd-city01"):
    try:
        print(str(num.select_one("a").text))
    except:
        pass
